...
BEGIN
  ...
  DROP FUNCTION IF EXISTS trigger_invitation_cleanup();
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_user_invitations_email') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_user_invitations_expires') THEN
