/*
  ...
  1. Tables
    - Creates auth_settings table for configuration
    - Stores key-value pairs with descriptions
  ...
  3. Functions
    - Adds get_auth_setting function
...
DROP POLICY IF EXISTS "Only super admins can manage auth settings" ON auth_settings;

-- Create RLS policy for super admins
CREATE POLICY "Only super admins can manage auth settings v2"
  ON auth_settings
  FOR ALL
  TO authenticated
